...
void loop() {
  ...
  if (1)  // Term.available())
  {
    int16_t inChar = 13;  //jh other inputTerm.get_key();
    if (inChar < 127)
    ...
    switch (inChar) {
      ...
      case 'Z':
        if (Throttle.getState() <= TH_ST_RELEASE) {
      ...
      case ']':
        if (recallIndex < (RECALL_BUFFER_SIZE - 1)) {
          recallIndex++;
      ...
      default:
        if (Throttle.getState() <= TH_ST_RELEASE) {
          ...
          //jh update for LCD printAddress(LocoAddr);
